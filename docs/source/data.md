...
1. Outcome Label - if text, will be numerically encoded (0 or 1) based on alphabetical order 
2. Instance Label - is commonly non-numeric and is not changed by STREAMLINE
3. Match Label - is commonly non-numeric and is not changed by STREAMLINE
4. Features - if text, will be numerically encoded based on alphabetical order and will automatically be treated as categorical features (overriding any user specification with `quantitative_feature_path` or `categorical_feature_path`)
