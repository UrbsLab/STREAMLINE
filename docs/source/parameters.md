...
| --timeout              | timeout                  | seconds until hyperparameter sweep stops running new trials (Note: it may run longer to finish last trial started) If set to None STREAMLINE is completely replicable but will take longer to run | 900 (900 sec/15 mins) |
