...
This is the easiest but most limited way to run STREAMLINE. These instructions are geared towards those with little to no computing experience. All other users can skip to the next [section](#use-mode-2-jupyter-notebook) but may wish to revisit later parts of this section for helpful details.
...
Outside of the notebook, navigate back to your Google Drive and reopen the folder: 'STREAMLINE/Colab_Output'. You should find the saved experiment folder that was output by the run, called 'hcc_demo'. Within this folder you should find the following:
    * File - 'hcc_demo_ML_Pipeline_Report.pdf': This is an automatically formatted PDF summarizing key findings during the model training and evaluation. A great place to start!
    * File - 'metadata.csv': Another way to view the pipeline run parameters used by the pipeline.  These are also organized on the first page of the PDF report.
    * File - 'metadata.pickle': A binary 'pickle' file of the metadata for easy loading by the different run phases. (For experienced users only)
    * File - 'algInfo.pickle': A binary 'pickle' file including a dictionary indicating which ML algorithms were used, along with abbreviations of names for figures/filenames, and colors to use for each algorithm in figures. (For experienced users only)
    * Folder - 'DatasetComparisons': Containing figures and statistical significance comparisons between the two datasets that were analyzed with STREAMLINE. (This folder only appears if more than one dataset was included in the user specified data folder, i.e. 'data_path')
    * Folders - A folder for each of the two datasets analyzed (in this demo there were two: hcc-data_example and hcc-data_example_no_covariates). These folders include all results and models respective to each dataset. We summarize the contents of each folder below (feel free to skip this for now and revisit it as needed)...
        * Folder - 'exploratory': Includes all exploratory analysis summaries and figures.
        * Folder - 'CVDatasets': Includes all individual training and testing datasets (as .csv files) generated.
        ...
        * Folder - 'scale_impute': Includes all pickled files preserving how scaling and/or imputation was conducted based on respective training datasets.
        * Folder - 'feature_selection': Includes feature importance and selection summaries and figures.
        * Folder - 'models': Includes the ML algorithm hyperparameters selected by Optuna for each CV partition and modeling algorithm, as well as pickled files storing all models for future use.
        * Folder - 'model_evaluation': Includes all model evaluation results, summaries, figures, and statistical comparisons.
        * Folder - 'applymodel': Includes all model evaluation results when applied to a hold out replication datasets. This includes a new PDF summary of models when applied to this further hold-out dataset.
        * File - 'runtimes.csv': Summary file giving the total runtimes spent on each phase or ML modeling algorithm in STREAMLINE.
...
2. Place 1 or more 'target' datasets in this folder following these guidelines:
...
5. Update the first 6 pipeline run parameters as such.
    1. demo_run: Change from True to False (without '').
    2. data_path: Change the end of the path from DemoData to the name of your new dataset folder (e.g. "/content/drive/MyDrive/STREAMLINE/my_data").
    3. output_path: This can be left 'as-is' or modified to some other folder on your google drive within which to store all STREAMLINE experiments.
    4. experiment_name: Change this to some new unique experiment name (do this each time you want to run a new experiment, either on the same or different dataset(s)), e.g. 'my_first_experiment'.
    5. class_label: Change to the column header indicating the class label in each dataset, e.g. 'Class'.
    6. instance_label: Change to the column header indicating instance ID's in each dataset, or change to 'None' if your dataset does not include instance IDs.
...
7. If you have a hold out replication dataset (with the same set of columns as one of the original 'target' datasets), you can apply the trained models to this new data for evaluation. If so, update the following run parameters.
    1. rep_data_path: Change the path to point to a different folder on Google Drive that contains one or more replication datasets intended to be applied to one of your original 'target' datasets.
    2. data_path_for_rep: Change to the path+filename of the 'target' dataset used to train the models we wish to apply the replication data to. This will be a specific file from the first dataset folder you created (e.g. my_data)
...
9. [Optional] Update other STREAMLINE run parameters to suit your analysis needs within code blocks 6-14. We will cover some common run parameters to consider here:
    1. cv_partitions: The number of CV training/testing partitions created, and consequently the number of models trained for each ML algorithm. We recommend setting this between 3-10. A larger value will take longer to run.
    2. categorical_cutoff: STREAMLINE uses this parameter to automatically determine which features to treat as categorical vs. numeric. If a feature has more than this many unique values, it is considered to be numeric.
    3. do_all: Setting this to 'True' will run all ML algorithms available by default, assuming no other do_ALGORITHM parameter is set to 'False'
    4. do_ALGORITHM: There are 15 possible algorithms (e.g. do_NB, do_XGB, etc).  Setting any of these to 'True' or 'False' will override the default setting and run or not run that specific algorithm.
    5. n_trials: Set to a higher value to give Optuna more attempts to optimize hyperparameter settings.
    6. timeout: Set higher to increase the maximum time allowed for Optuna to run the specified N_trials (useful for algorithms that take more time to run)
...
2. Open jupyter notebook (https://jupyter.readthedocs.io/en/latest/running.html). We recommend opening the 'anaconda prompt' which comes with your anaconda installation.  Once opened, type the command 'jupyter notebook' which will open as a webpage. Navigate to your working directory and open the included jupyter notebook file: 'STREAMLINE-Notebook.ipynb'.
