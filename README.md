...
```
pip install skrebate==0.7 xgboost lightgbm catboost gplearn scikit-eLCS scikit-XCS scikit-ExSTraCS optuna==2.0.0 plotly kaleido==0.0.3.post1 fpdf  
```

* For Anaconda3-2022.05-Linux-x86_64 or other newer Anaconda versions (not tested) use:
```
pip install skrebate==0.7 xgboost lightgbm catboost gplearn scikit-eLCS scikit-XCS scikit-ExSTraCS optuna==2.0.0 plotly kaleido fpdf scipy  
```
...
* xgboost (v_1.6.1) Extreme Gradient Boosting ML classification algorithm

* lightgbm (v_3.3.2) Light Gradient Boosting ML classification algorithm
...
* scikit-ExSTraCS (v_1.1.1) Extended Supervised Learning and Tracking Classifier System ML algorithm (Developed and Implemented by our lab)

* optuna (v_2.0.0) Optuna, a hyperparameter optimization framework

* plotly (v_5.6.0) An open-source, interactive data visualization library. Used by Optuna to generate hyperparameter sweep visualizations
...
python ApplyModelMain.py --out-path /myoutputpath --exp-name hcc_demo --rep-data-path /myrepdatapath/STREAMLINE/DemoRepData  --dataset /mydatapath/STREAMLINE/DemoData/hcc-data_example.csv --run-parallel False

python PDF_ReportMain.py --training False --out-path /myoutputpath --exp-name hcc_demo --rep-data-path /myrepdatapath/STREAMLINE/DemoRepData  --dataset /mydatapath/STREAMLINE/DemoData/hcc-data_example.csv --run-parallel False
...
python ApplyModelMain.py --out-path /myoutputpath --exp-name hcc_demo --rep-data-path /myrepdatapath/STREAMLINE/DemoRepData  --dataset /mydatapath/STREAMLINE/DemoData/hcc-data_example.csv

python PDF_ReportMain.py --training False --out-path /myoutputpath --exp-name hcc_demo --rep-data-path /myrepdatapath/STREAMLINE/DemoRepData  --dataset /mydatapath/STREAMLINE/DemoData/hcc-data_example.csv
...
| --out-path | path to output directory | MANDATORY |
| --exp-name | name of experiment output folder (no spaces) | MANDATORY |
| --max-feat | max features to keep (only applies if filter_poor_features is True) | 2000 |
...
| --n-trials | # of bayesian hyperparameter optimization trials using optuna (specify an integer or None) | 200 |
| --timeout | seconds until hyperparameter sweep stops running new trials (Note: it may run longer to finish last trial started) If set to None, STREAMLINE is completely replicable, but will take longer to run | 900 |
...
The most common reasons for failures/errors are formatting problems involving the target dataset(s), and incompatibility issues with the installed version of Anaconda and other key python packages. We recommend users first check the following when running into STREAMLINE run completion issues:
1. Ensure all datasets are properly formatted as specified in: [Running STREAMLINE on your own dataset(s)](#running-streamline-on-your-own-datasets)
2. Carefully check the version of Anaconda3 as well as other required python packages as specified in: [Prerequisites](#prerequisites)
...
1. Try to kill the given job(s) and use the `-r` command for `ModelMain.py`.  When using this command, a different random seed will automatically which can resolve the run completion.
