...
def _make_cv_dataset(
    root: Path,
    dataset_name: str = "ToyData",
    n_features: int = 8,
    n_samples: int = 300,
):
    """
    Creates <root>/<dataset_name>/CVDatasets with a single CV split (0)
...
def _assert_artifacts(dataset_dir: Path, small_name: str):
    """
    Verifies pickled model + JSON metrics and a sensible probability
    interface post-calibration.
    """
    # pickled model
    ...
    # JSON metrics (new layout)
    metrics_json = (
        dataset_dir
        / "model_evaluation"
        / "metrics_by_cv"
        / f"{small_name}_CV_0.json"
    )
    assert metrics_json.exists(), f"Missing metrics JSON for {small_name}"

    with open(metrics_json, "r") as f:
        payload = json.load(f)

    # We expect either a nested {"metrics": {...}} or flat dict with metric keys
    if "metrics" in payload:
        m = payload["metrics"]
    else:
        m = payload

    # At minimum, balanced_accuracy must be present
    assert "balanced_accuracy" in m, f"Expected 'balanced_accuracy' in metrics for {small_name}"

    # load and inspect calibration / proba
    with open(model_pkl, "rb") as f:
        est = pickle.load(f)

    # The calibrated estimator is either a CalibratedClassifierCV,
    # or exposes predict_proba
    ...
    assert has_proba or is_calibrated, (
        f"Expected calibrated or proba-capable model for {small_name}"
    )
...
def test_p6_modeling_smoke(phase6_layout, models_csv, expected_small_names):
    ...
    Verifies model/metrics artifacts and that the estimator is calibrated or
    proba-capable.
    """
