...
def test_prc_curve_interpolation_collapses_duplicate_recall_to_best_precision():
    ...
    assert np.allclose(sorted_recall, np.array([0.0, 0.5, 1.0]))
    assert np.allclose(sorted_precision, np.array([1.0, 0.8, 0.2]))
