...
def test_valid_kfold():
    partition_method, experiment_path = "Random", "./tests/"
    kfold = KFoldPartitioner(dataset_valid, partition_method, experiment_path)
    train_dfs, test_dfs = kfold.cv_partitioner(n_splits=5, random_state=42, return_dfs=False, save_dfs=False,
                                               partition_method=partition_method)
    assert (train_dfs is None)
    assert (test_dfs is None)
    train_dfs, test_dfs = kfold.cv_partitioner(n_splits=5, random_state=42, return_dfs=True, save_dfs=False,
                                               partition_method=partition_method)
