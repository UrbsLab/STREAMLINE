...
def _maybe_int(s): return None if s in (None,"") else int(s)
def _maybe_float(s): return None if s in (None,"") else float(s)

def main():
    ap = argparse.ArgumentParser("P4 feature-importance single-CV jobsubmit")
    ...
    FeatureImportance(
        ...
        model_params=_maybe_json(args.model_params),
        ...
        instance_subset=_maybe_int(args.instance_subset),
    ).run()
